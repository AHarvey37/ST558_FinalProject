...
The following variables are continuous in the dataset: 1. BMI 2. GenHlth 3. MentHlth 4. PhysHlth 5. Age 6. Education 7. Income
...
Means<- rawData|>
  ...
  summarise(Diabetes_binary.mean = mean(Diabetes_binary),
            HighBP.mean = mean(HighBP),
            HighChol.mean = mean(HighChol),
            HvyAlcoholConsump.mean = mean(HvyAlcoholConsump),
            Smoker.mean = mean(Smoker),
            PhysActivity.mean = mean(PhysActivity),
            AgeGroup.mean = mean(Age),
            HeartDiseaseorAttack.mean = mean(HeartDiseaseorAttack),
            MentHlth.mean = mean(MentHlth))|>
